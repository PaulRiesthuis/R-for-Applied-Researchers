...
```{r, include = knitr::is_html_output(), message=F}
library(reactable)

table_react <- reactable(
  df_analysis,
  searchable = TRUE,
  pagination = TRUE,
  defaultPageSize = 5,
...
```{r, include = knitr::is_html_output(), message= F, warning=F}
library(ggstatsplot)

plot_between_stats <- ggbetweenstats(
  data  = df_analysis,
  x     = condition,
  y     = overall_belief,
)
...
plot_facet <- ggplot(df_long, aes(x = condition, y = score, fill = condition)) +
